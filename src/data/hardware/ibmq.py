...
class IBMQHardware:
    ...
    def _init_service(self):
        """Initialize IBMQ service with token."""
        if self._service is None:
            try:
                ...
                print(f"✅ IBMQ service initialized")
            except Exception as e:
                print(f"⚠️ IBMQ service error: {e}")
                print("   Using Aer simulator as fallback.")
                self._service = None
    ...
    def backend(self):
        if self._backend is None and self.service is not None:
            try:
                ...
                print(f"⚠️ Backend {self.backend_name} error: {e}")
                self._backend = None
    ...
    def _run_simulator(self, circuit: QuantumCircuit, shots: int) -> Dict[str, int]:
        print(f"   💻 Running on Aer simulator...")
