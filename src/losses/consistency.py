...
class ConsistencyLoss(nn.Module):
    ...
    Encourages the HN-E head to produce consistent results whether given
    a raw low-shot input or the SN-D denoised input:
    
    L_consist = TVD(f_HN(f_SN(x_low)), f_HN(x_high))
    
    This ties the two stages together.
    """
    ...
    def __init__(
        ...
        """
        Args:
            method: 'tvd', 'kl', or 'both'
            weight: Overall weight for consistency loss
        """
        super().__init__()
        self.method = method
        self.weight = weight
    ...
    def forward(
        self,
        hn_on_sn: torch.Tensor,
        hn_on_raw: torch.Tensor,
        reduction: str = 'mean',
    ) -> torch.Tensor:
        """
        Compute consistency loss between two HN-E outputs.
        
        Args:
            hn_on_sn: HN-E output when applied to SN-D denoised input
            hn_on_raw: HN-E output when applied to raw high-shot input
...
class CrossStageConsistency(nn.Module):
    ...
    Ties the SN-D and HN-E stages together for joint training.
    """
    
    def __init__(
        self,
        tvd_weight: float = 0.5,
        kl_weight: float = 0.5,
        consistency_weight: float = 0.3,
    ):
        """
        Args:
            tvd_weight: Weight for TVD term
            kl_weight: Weight for KL term
            consistency_weight: Overall consistency weight
        """
        super().__init__()
    ...
    def forward(
        ...
        """
        Compute cross-stage consistency loss between SN-D and HN-E heads.
...
class JointConsistencyLoss(nn.Module):
    ...
    Combines:
    1. Cross-stage consistency between SN-D and HN-E heads
    2. Temporal consistency across training steps (optional)
    """
    
    def __init__(
        self,
        cross_stage_weight: float = 0.3,
        ...
        """
        Args:
            cross_stage_weight: Weight for cross-stage consistency
            temporal_weight: Weight for temporal consistency (not yet implemented)
            use_kl: Whether to use KL divergence
            use_tvd: Whether to use TVD
        """
        super().__init__()
    ...
    def forward(
        ...
        sn_dist: torch.Tensor,
        hn_dist: torch.Tensor,
        prev_sn_dist: torch.Tensor = None,
        prev_hn_dist: torch.Tensor = None,
    ) -> torch.Tensor:
        """
        Compute joint consistency loss.
        
        Args:
            sn_dist: (B, M) SN-D distribution
            hn_dist: (B, M) HN-E distribution
            prev_sn_dist: Previous SN-D distribution (for temporal consistency)
            prev_hn_dist: Previous HN-E distribution (for temporal consistency)
        ...
        if self.use_kl or self.use_tvd:
            ...
            if self.use_kl and self.use_tvd:
                ...
                kl = 0.5 * (
                    (sn_clamped * (sn_clamped / hn_clamped).log()).sum(dim=-1).mean()
                ...
                loss = loss + self.cross_stage_weight * (
                    (sn_clamped * (sn_clamped / hn_clamped).log()).sum(dim=-1).mean()
                )
        
        # Temporal consistency (if previous distributions are provided)
